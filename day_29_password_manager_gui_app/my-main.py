...
def validate_password(username: str | int, pwd: str) -> Tuple[bool, str]:
    ...
    with open("pass_data.tsv", newline="") as file:
        ...
        for row in reversed(list(csv.reader(file, delimiter="\t"))):
            if len(row) == 3 and username == row[1]:
                if row[2].lower() == pwd.lower():
...
def add_data():
    ...
    if confirmation.lower() == "yes":
        with open("pass_data.tsv", mode="a", newline="") as file:
...
website_input.grid(row=1, column=1, padx=20, pady=5)

email_label = Label(text="Email/Username:")
email_label.grid(row=2, column=0)

email_input = Entry(width=20)
email_input.grid(row=2, column=1, padx=20, pady=5)

password_label = Label(text="Password:")
password_label.grid(row=3, column=0)

password_input = Entry(width=20)
password_input.grid(row=3, column=1, padx=20, pady=5)

gen_pass_btn = Button(text="Generate Password", command=set_password)
gen_pass_btn.grid(row=3, column=2)

add_button = Button(
    text="Add", width=15, bg="blue", fg="white", font=("", 10, "bold"),
    command=add_data)
